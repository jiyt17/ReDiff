...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.vision_resampler = build_vision_resampler(config, vision_tower=self.vision_tower) # IdentityMap()
            self.mm_projector = build_vision_projector(config, vision_cfg=self.vision_tower.config)

            if "unpad" in getattr(config, "mm_patch_merge_type", ""): # spatial_unpad
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        mm_patch_merge_type = model_args.mm_patch_merge_type # spatial_unpad
        ...
        self.config.mm_projector_type = getattr(model_args, "mm_projector_type", "linear") # mlp2x_gelu
        ...
        if pretrain_mm_mlp_adapter is not None: # False
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None, is_llada=False):
        ...
        # import pdb; pdb.set_trace()
        if type(images) is list or images.ndim == 5: # [tensor[5, 3, 384, 384]]
            ...
            # print('encoded_image_features', encoded_image_features.shape) # [5, 729, 4096]
