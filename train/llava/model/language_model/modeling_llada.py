...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        cfg_scale=0., remasking='low_confidence', mask_id=126336, tokenizer=None, stopping_criteria=None, generation_suffix=None, revise=False, revise_start=1.0, revise_topk=None, fake_ans=None, **kwargs):
        '''
        Args:
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # add wrong answer for revision
            if fake_ans is not None:
                fake_ans_embeds = self.model.embed_tokens(fake_ans)  # (1, s, d)
                # print(fake_ans_embeds.shape)
                x_embeds[:, inputs_embeds.shape[1]:inputs_embeds.shape[1] + fake_ans_embeds.shape[1]] = fake_ans_embeds.clone()
                x[:, inputs_embeds.shape[1]:inputs_embeds.shape[1] + fake_ans_embeds.shape[1]] = fake_ans
            ...
            stop_tokens = []
            if stopping_criteria is not None: # <|eot|>
            ...
            for num_block in range(num_blocks):
                ...
                revise_start_step = steps - math.ceil(steps * revise_start)
                for i in range(steps):
                    # print(i, 'input', tokenizer.decode(x[0, inputs_embeds.shape[1]:]))
                    ...
                    # if i % 4 == 0:
                        # print(i, 'output', x0[:, inputs_embeds.shape[1]:])
                    ...
                    # print('i', i, 'all responses:', x0_p[-gen_length:].cpu().numpy().tolist())
                    confidence = torch.where(mask_index, x0_p, -np.inf)
                    ...
                    if revise and i >= revise_start_step:
                        # print('mask', mask_index)
                        block_unmask_index = ~mask_index
                        block_unmask_index[:, :block_start] = False
                        block_unmask_index[:, block_end:] = False
                        # print('unmask', block_unmask_index)
                        # set top k unmasked to revise
                        if revise_topk:
                            unmask_confidence = torch.where(block_unmask_index, x0_p, -np.inf)
                            unmask_length = torch.min(torch.sum(block_unmask_index, dim=1)) # todo multi-sentence
                            topk_unmask_index = torch.topk(unmask_confidence, k=int(unmask_length*revise_topk), dim=-1).indices
                            block_unmask_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
                            block_unmask_index.scatter_(1, topk_unmask_index, True)

                        # threshold_index = x0_p > 0.7
                        # block_unmask_index = block_unmask_index & threshold_index
                        x = torch.where(block_unmask_index, x0, x)
                        x_embeds = torch.where(block_unmask_index.unsqueeze(-1).expand_as(x_embeds), x0_embeds, x_embeds)
    ...
    #             unrevise_ind = ~ revise_indices
    #             masked_indices = unrevise_ind & masked_indices
    ...
    #     noisy_embeds, p_mask, masked_embed = forward_process_embeds(inputs_embeds, labels, revise_indices)
    ...
    #         revise_indices_randn = torch.rand((b, l), device=inputs_embeds.device) < 0.05
    #         revise_indices_randn = revise_indices_randn & valid_mask & unmasked_indices # shape (b, l)
    #         if revise_indices is not None:
    #             unrevise_ind = ~ revise_indices
    #             revise_indices_randn = unrevise_ind & revise_indices_randn
    #         revise_matrix = torch.randint(1, self.vocab_size-1000, (b, l), device=inputs_embeds.device)
    #         revise_embeds = self.model.embed_tokens(revise_matrix) # shape (b, l, d)
    #         noisy_embeds = torch.where(revise_indices_randn.unsqueeze(-1), revise_embeds, noisy_embeds) # shape (b, l, d)
    #     if revise_indices is not None:
    #         revise_indices = revise_indices | revise_indices_randn
    #     else:
    #         revise_indices = revise_indices_randn
    ...
    #             # masked_indices = masked_indices | revise_indices # shape (b, l)
    #             # token_loss = F.cross_entropy(logits[masked_indices], labels[masked_indices], ignore_index=-100,
    #             #                             reduction='none') / p_mask[masked_indices]
    #             # loss = torch.sum(token_loss / noisy_data_length[masked_indices]) / labels.shape[0]

    #             # normal with each length
    #             mask_length = torch.sum(masked_indices, dim=-1, keepdim=True) # shape (b, 1)
    #             mask_length = mask_length.repeat(1, noisy_embeds.shape[1]) # shape (b, l)
    #             token_loss = F.cross_entropy(logits[masked_indices], labels[masked_indices], ignore_index=-100,
    #                                         reduction='none') / p_mask[masked_indices]
    #             mask_loss = torch.sum(token_loss / mask_length[masked_indices]) / labels.shape[0]
    #             # print('mask', loss)
    #             revise_length = torch.sum(revise_indices, dim=-1, keepdim=True) # shape (b, 1)
    #             revise_length = revise_length.repeat(1, noisy_embeds.shape[1]) # shape (b, l)
    #             token_loss = F.cross_entropy(logits[revise_indices], labels[revise_indices], ignore_index=-100,
    #                                         reduction='none') / p_mask[revise_indices]
    #             revise_loss = torch.sum(token_loss / revise_length[revise_indices]) / labels.shape[0]
    #             # print('revise', loss)
    #             valid_indices = (labels != -100) & ~masked_indices & ~revise_indices # shape (b, l)
    #             valid_length = torch.sum(valid_indices, dim=-1, keepdim=True) # shape (b, 1)
    #             valid_length = valid_length.repeat(1, noisy_embeds.shape[1]) # shape (b, l)
    #             token_loss = F.cross_entropy(logits[valid_indices], labels[valid_indices], ignore_index=-100,
    #                                         reduction='none') / p_mask[valid_indices]
    #             others_loss = torch.sum(token_loss / valid_length[valid_indices]) / labels.shape[0]
    #             # print('others', loss)
    #             loss = mask_loss + revise_loss + others_loss
    ...
    def forward(
        ...
        def forward_process_embeds(input_embeds, labels, revise_indices, eps=1e-3):
            ...
            if revise_indices is not None:
                for i in range(b):
                    if torch.sum(revise_indices[i]) > 0:
        ...
        # else:
        #     revise_indices = revise_indices_randn
        # print('mask_indices', masked_indices, masked_indices.sum(dim=1))
        noisy_data_length = torch.sum((1-prompt_index), dim=-1, keepdim=True) # shape (b, 1)
        noisy_data_length = noisy_data_length.repeat(1, noisy_embeds.shape[1]) # shape (b, l)
        ...
        if labels is not None:
            ...
            if not revise:
                ...
                for i in range(b):
                    if torch.sum(revise_indices[i]) == 0:
                        mask_length = torch.sum(masked_indices[i], dim=-1, keepdim=True) # shape (b, 1)
                        mask_length = mask_length.repeat(noisy_embeds.shape[1]) # shape (b, l)
                        token_loss = F.cross_entropy(logits[i][masked_indices[i]], labels[i][masked_indices[i]], ignore_index=-100,
                                                    reduction='none') / p_mask[i][masked_indices[i]]
                        mask_loss = torch.sum(token_loss / mask_length[masked_indices[i]])
                        # print('mask', loss)
                        revise_length = torch.sum(revise_indices_randn[i], dim=-1, keepdim=True) # shape (b, 1)
                        revise_length = revise_length.repeat(noisy_embeds.shape[1]) # shape (b, l)
                        token_loss = F.cross_entropy(logits[i][revise_indices_randn[i]], labels[i][revise_indices_randn[i]], ignore_index=-100,
                                                    reduction='none') / p_mask[i][revise_indices_randn[i]]
                        revise_loss = torch.sum(token_loss / revise_length[revise_indices_randn[i]])
                        # print('revise', loss)
                        valid_indices = ((labels != -100) & ~masked_indices & ~revise_indices_randn)[i] # shape (b, l)
                        valid_length = torch.sum(valid_indices, dim=-1, keepdim=True) # shape (b, 1)
                        valid_length = valid_length.repeat(noisy_embeds.shape[1]) # shape (b, l)
                        token_loss = F.cross_entropy(logits[i][valid_indices], labels[i][valid_indices], ignore_index=-100,
                                                    reduction='none') / p_mask[i][valid_indices]
                        others_loss = torch.sum(token_loss / valid_length[valid_indices])
                        # print('others', loss)
                        losses.append(mask_loss + revise_loss + others_loss)
                    else:
                        revise_length = torch.sum(revise_indices[i], dim=-1, keepdim=True) # shape (b, 1)
                        revise_length = revise_length.repeat(noisy_embeds.shape[1]) # shape (b, l)
                        token_loss = F.cross_entropy(logits[i][revise_indices[i]], labels[i][revise_indices[i]], ignore_index=-100,
                                                    reduction='none') / p_mask[i][revise_indices[i]] # 不应该除p_mask了
                        revise_loss = torch.sum(token_loss / revise_length[revise_indices[i]])
    ...
    #         masked_indices = masked_indices & valid_mask # Combine random encoding and valid encoding
    #         if revise_indices is not None:
    #             masked_indices = masked_indices & (~revise_indices)
    #         # Magic number 126336 stands for the tokenizer special token,
    #         # Magic embeddings, which is used for [MASK] token here,
    #         masked_embed = self.model.embed_tokens(torch.tensor([126336]).to(input_embeds.device))
    #         noisy_embeds = torch.where(masked_indices.unsqueeze(-1), masked_embed, input_embeds)
    ...
    #         unmasked_indices = ~ masked_indices
    #         valid_mask = (labels != -100)
    #         revise_indices_randn = torch.rand((b, l), device=inputs_embeds.device) < 0.1
    #         # revise with t
    #         # revise_indices_randn = torch.rand((b, l), device=inputs_embeds.device) < p_mask
    #         # for i in range(p_mask.shape[0]):
    #         #     if p_mask[i, 0] > 0.1:
    #         #         revise_indices_randn[i] = False
    #         revise_indices_randn = revise_indices_randn & valid_mask & unmasked_indices # shape (b, l)
    #         if revise_indices is not None:
    #             revise_indices_randn = revise_indices_randn & (~revise_indices)
    #         revise_matrix = torch.randint(1, self.vocab_size-1000, (b, l), device=inputs_embeds.device)
    #         revise_embeds = self.model.embed_tokens(revise_matrix) # shape (b, l, d)
    #         noisy_embeds = torch.where(revise_indices_randn.unsqueeze(-1), revise_embeds, noisy_embeds) # shape (b, l, d)
    ...
    #             # revise_loss = torch.sum(token_loss / revise_length[revise_indices]) / labels.shape[0]
    #             # print('revise', loss)
    #             revise_length = torch.sum(revise_indices_randn, dim=-1, keepdim=True) # shape (b, 1)
    #             revise_length = revise_length.repeat(1, noisy_embeds.shape[1]) # shape (b, l)
    #             token_loss = F.cross_entropy(logits[revise_indices_randn], labels[revise_indices_randn], ignore_index=-100,
    #                                         reduction='none') / p_mask[revise_indices_randn]
    #             revise_randn_loss = torch.sum(token_loss / revise_length[revise_indices_randn]) / labels.shape[0]
    #             # print('revise_randn', loss)
    #             valid_indices = ((labels != -100) & ~masked_indices & ~revise_indices) # shape (b, l)
    #             valid_length = torch.sum(valid_indices, dim=-1, keepdim=True) # shape (b, 1)
    #             valid_length = valid_length.repeat(1, noisy_embeds.shape[1]) # shape (b, l)
    #             token_loss = F.cross_entropy(logits[valid_indices], labels[valid_indices], ignore_index=-100,
    ...
    #             others_loss = torch.sum(token_loss / valid_length[valid_indices]) / labels.shape[0]
    #             # print('others', loss)
    #             loss = mask_loss + revise_randn_loss + others_loss #+ revise_loss
