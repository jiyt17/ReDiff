...
class LlavaLLaDAModelLM(LLaDAModelLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        # print('after prepare, inputs:', inputs_embeds.shape, labels.shape if labels is not None else None) # torch.Size([1, 3448, 4096]) torch.Size([1, 3448])
        # print(labels[0].tolist())
        seq_len = labels.shape[1]
        pad_mask = torch.zeros([labels.shape[0], seq_len-revise_indices.shape[1]], dtype=bool, device=labels.device)
        revise_indices = torch.cat([pad_mask, revise_indices], dim=1)

        # rank0_print('input', labels[revise_indices])
    ...
    def generate(
        ...
        revise_topk = None,
        revise_start = 1.0,
        fake_ans: Optional[torch.Tensor] = None,
        **kwargs,
