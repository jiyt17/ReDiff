    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if revise_flag:
            ...
            for revise in revises:
                ...
                org_ids = self.tokenizer.encode('dummy ' + org)[1:]
                target_ids = self.tokenizer.encode('dummy ' + target)[1:]
                # print('org', org_ids)
                # assert len(org_ids) >= len(target_ids)
                if len(org_ids) != len(target_ids):
                    continue
                # if len(target_ids) < len(org_ids):
                #     target_ids = target_ids + [pad_id] * (len(org_ids)-len(target_ids))

                # match
                hal_st = -1
                for j in range(len(data_dict["input_ids"])):
                    if data_dict["input_ids"][j] == org_ids[0]:
                        match = True
                        for t in range(len(org_ids)):
                            if data_dict["input_ids"][j+t] != org_ids[t]:
                                match = False
                                break
                        if match:
                            hal_st = j
                            break
                if hal_st > 0:
                    revise_indices[hal_st: hal_st+len(org_ids)] = True
                    # label1 = data_dict['labels'].clone()
                    data_dict['labels'][hal_st: hal_st+len(org_ids)] = torch.tensor(target_ids).clone()
                    # label2 = data_dict['labels']
                    # print(label1 != label2)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "is_llada" in instances[0] and instances[0]["is_llada"]:
            ...
            # print('pad', input_ids.shape, labels.shape)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.use_pos_skipping is not None and model_args.pos_skipping_range is not None: # position embedding skipping: False,4096
        overwrite_config["use_pos_skipping"] = model_args.use_pos_skipping
        overwrite_config["pos_skipping_range"] = model_args.pos_skipping_range

    if model_args.rope_scaling_factor is not None and model_args.rope_scaling_type is not None: # False
    ...
    if model_args.mm_spatial_pool_stride is not None and model_args.mm_spatial_pool_out_channels is not None and model_args.mm_spatial_pool_mode is not None and model_args.mm_resampler_type is not None: # False
        ...
        overwrite_config["mm_spatial_pool_mode"] = model_args.mm_spatial_pool_mode # bilinear
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.image_aspect_ratio = data_args.image_aspect_ratio # anyres_max_4
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
                ...
                assert patch_size in [224, 336, 384, 448, 512], "patch_size should be in [224, 336, 384, 448, 512]" # 384
                # Use regex to extract the range from the input string
                matches = re.findall(r"\((\d+)x(\d+)\)", data_args.image_grid_pinpoints)
                range_start = tuple(map(int, matches[0])) # (1,1)
                range_end = tuple(map(int, matches[-1])) # (6,6)
                # Generate a matrix of tuples from (range_start[0], range_start[1]) to (range_end[0], range_end[1]) 6x6 matrix
                grid_pinpoints = [(i, j) for i in range(range_start[0], range_end[0] + 1) for j in range(range_start[1], range_end[1] + 1)]
                # Multiply all elements by patch_size
                data_args.image_grid_pinpoints = [[dim * patch_size for dim in pair] for pair in grid_pinpoints] # all possible image sizes [384, 384], [384, 768], [384, 1152], [384, 1536]...
            elif isinstance(data_args.image_grid_pinpoints, str):
                data_args.image_grid_pinpoints = ast.literal_eval(data_args.image_grid_pinpoints)

        model.config.image_grid_pinpoints = data_args.image_grid_pinpoints
        model.config.image_crop_resolution = data_args.image_crop_resolution # None
        model.config.image_split_resolution = data_args.image_split_resolution # None
        ...
        rank0_print(f"Trainable parameters: ~{trainable_params/1e6:.2f} MB)") # full tuning
        if training_args.bits in [4, 8]:
